...
export default function ModalResult({ closeModal }) {
  ...
  //à l'ouverture de la modale, on desactive le scroll
  useEffect(() => {
    document.body.style.overflowY = "hidden"

    //cleanup fonction (s'execute quand le composant est détruit)
  ...
  return (
    <div
      ...
      <div
        ...
        className="max-w-[800px] rounded p-7 bg-gray-50 mb-[10vh]">
        <div className="flex items-end mb-5">
          <p className="font-semibold mr-5">Here is your code</p>
          <button className="ml-auto mr-2 text-sm bg-green-500 text-white hover:bg-green-700 py-1 px-3 rounded "
          onClick={handleCopy}>Copy</button>
          <button
            onClick={closeModal}
            className="text-sm bg-red-500 text-white hover:bg-red-700 py-1 px-3 rounded ">Close</button>
        </div>
